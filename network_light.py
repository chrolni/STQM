...
class Senseiver(pl.LightningModule):
    def __init__(self,**kwargs):
        ...
        self.encoder = Encoder(
            input_ch = self.hparams.im_ch+pos_encoder_ch + 96,
    ...
    def test(self, dataloader, num_pix=64, split_time=10):
        ...
        hidden = dataloader.dataset.hidden
        ...
        for t_start in bar( times[1:] ):
            ...
            for pix in bar( pixels ):
                ...
                hidden_coords = hidden[t:t_start,]
                hidden_coords = hidden_coords.flatten(start_dim=2, end_dim=-1)[:,:,pix:pix+num_pix].transpose(1, 2)
                ...
                hidden_sensor = hidden[t:t_start,]
                hidden_sensor = hidden_sensor.flatten(start_dim=2, end_dim=-1)[:,:,sensors].transpose(1, 2)
                sensor_values = torch.cat([sensor_values,hidden_sensor], axis=-1)

                out = self(sensor_values, coords) # encode,query
