...
def main():
    ...
    if not data_config['test']:
        ...
        if data_config['gpu_device']:
            ...
            dataloader.dataset.hidden = torch.as_tensor(
                dataloader.dataset.hidden).to(f"cuda:{device}")

        path = model_loc.split('checkpoints')[0]

        with torch.no_grad():
            output_im, im, sensors = model.test(dataloader, num_pix=64, split_time=10)
        plot_cs(output_im,im,sensors)
